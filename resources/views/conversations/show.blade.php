        ...
        <div class="col-sm-8">
            <div class="card">
                <div class="card-header">
                    <a href="{{ action('UsersController@show', $user->slug) }}">
                        @if(!$user->isBlackListed())
                            {{$user->name}}
                        @else
                            <i>Utilisateur mis en liste noire...</i>
                        @endif
                    </a>
                </div>
                        ...
                        <div class="scrollContent">
                            @if(!$user->isBlacklisted())
                                ...
                                @foreach($messagess as $message)
                                    ...
                                    <?php $index++; ?>
                                @endforeach
                                @if(\Illuminate\Support\Facades\Auth::user()->AmIBlacklisted($user))
                                        <p><em>Vous avez été bloqué{{ \Illuminate\Support\Facades\Auth::user()->sex == "femme" ? "e" : "" }} par {{ $user->name }}</em></p>
                                @endif
                                @if($messages->hasMorePages())
                                    <div class="text-center">
                                ...
                                <br>
                            @else
                                <i>Utilisateur mis dans la liste noire</i>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="card-footer">
                    <form action="" method="post" class="form-horizontal">
                        ...
                        <div class="form-group">
                            <textarea name="content" id="" class="form-control" placeholder="Ecrivez votre message" {{ $user->isBlackListed() ? 'disabled' : '' }} {{ \Illuminate\Support\Facades\Auth::user()->AmIBlacklisted($user) ? 'disabled' : '' }}></textarea>
                        </div>
                        <div class="form-group">
