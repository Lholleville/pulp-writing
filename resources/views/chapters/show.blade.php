...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            @if($chapter->avatar != '/img/chapters/defaut.jpg')
                <img src="{{ url($chapter->avatar) }}" alt="Illustration de {{ $chapter->name }}" class="img-illu mx-auto d-block">
            @endif
        </div>
    </div>
    <br>
    <div class="row">
        <div class="col-sm-12">
            <h2 class="text-center">{{ $chapter->POV }}</h2>
        </div>
    </div>
    ...
    <div class="row offset-sm-3">
        ...
        <div class="col-sm-2 text-center"><i class="far fa-comments"></i><br>{{ $chapter->nbComments }}</div>
        <div class="col-sm-2 text-center"><i class="fas fa-pencil-alt"></i><br>{{ $chapter->words }}</div>
    </div>
    ...
    <div class="row">
        <div class="col-sm-12">
            @if(Auth::user())
                <div id="annotation_icon"><button class="btn btn-default" data-toggle="modal" data-target="#annotation_modal"><i class="far fa-comment-dots"></i></button></div>
            @endif
            <div class="text-justify" id="chapitre">{!! $chapter->content !!}</div>
        </div>
    </div>
